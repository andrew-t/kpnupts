...
function checkGamepad() {
	...
	function control(player, pad, stick) {
		...
		if (upDown < -stickDeadZone)
			deaden(stickDeadTime,
				player + ',' + pad + ',' + stick + ',up',
				() => cursor.moveY(-1));
		else if (upDown > stickDeadZone)
			deaden(stickDeadTime,
				player + ',' + pad + ',' + stick + ',down',
				() => cursor.moveY(1));
		if (leftRight < -stickDeadZone)
			deaden(stickDeadTime,
				player + ',' + pad + ',' + stick + ',left',
				() => cursor.moveX(-1));
		else if (leftRight > stickDeadZone)
			deaden(stickDeadTime,
				player + ',' + pad + ',' + stick + ',right',
				() => cursor.moveX(1));
	}
}
