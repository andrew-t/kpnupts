function startGame() {
	...
	// Initialise
	var grid = [];
	for (let i = 0; i < cols; ++i) grid.push(new Set());
	// Pre-populate pusher
	grid.forEach(col => {
		for (let y = 0; y < cursorAreaTop + preblockRows; ++y) col.add(randomTile(y));
	});

	var frameRequestId = window.requestAnimationFrame(frameCalculator),
	    game = {
		startTime: Date.now(),
		grid,
		stop,
	...
	function frameCalculator(now) {
		frameRequestId = window.requestAnimationFrame(frameCalculator);
...
function calculateNewFrame(game, now) {
	...
	if (delta > maxDeltaPerFrame) delta = maxDeltaPerFrame;
	lastFrame = now;

	if (!game.pusherStateChange) game.pusherStateChange = now;else {
		...
		if (!game.pusherDirection) {
			...
			if (now - game.pusherStateChange >= pusherPause) {
				game.pusherDirection = game.pusherPosition == 0 ? down : up;
			...
			if (game.pusherPosition < 0) {
				...
				game.grid.forEach(col => {
					...
					for (let i = 0; i < blocks; ++i) col.add(randomTile(-1 - i));
				});
			...
			function stopPusher() {
				...
				game.grid.forEach(col => {
					for (let block of col) {
						...
						if (block.y > pusherBottom - 0.5) block.onPusher = false;
					}
				});
			}
			...
			game.grid.forEach(column => {
				for (let block of column) if (block.onPusher) {
					block.y += pusherDelta;
					if (game.pusherDirection == up && block.y < 0) block.y = 0;
				} else if (block.y < pusherBottom) block.y = pusherBottom;
			});
		}
	}
	...
	game.grid.forEach(column => {
		for (let block of column) if (block.y >= height - 0.5) {
			column.delete(block);
			if (++game.drops > allowedDrops) game.stop();
			game.cursors.forEach(cursor => {
				if (block == cursor.block) cursor.moveY(up);
			});
		} else block.lastY = block.y;
	});
